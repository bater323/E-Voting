...
class m_login extends CI_Model{
	...
	// // $hasil=$this->db->query("SELECT * FROM tbl_pengguna WHERE pengguna_username='admin1'")->result();
    //     return $hasil;
	// }
	...
	// $hasil=$this->db->query("SELECT * FROM tbl_pengguna_sekolah WHERE pengguna_username='admin1'")->result();
	 ...
	 function cekuser($username,$password){
        $username=$this->db->escape($username);
        $password=$this->db->escape($password);
	   $hasil=$this->db->query("SELECT * FROM tblregister WHERE no_anggota=$username AND password_user=$password");
	...
	function simpan_pengguna($nama,$jenkel,$username,$password,$email,$nohp,$level,$gambar){
		$hsl=$this->db->query("INSERT INTO tbl_pengguna (pengguna_nama,pengguna_jenkel,pengguna_username,pengguna_password,pengguna_email,pengguna_nohp,pengguna_level,pengguna_photo) 
		VALUES ('$nama','$jenkel','$username',md5('$password'),'$email','$nohp','$level','$gambar')");
		return $hsl;
	}

	function simpan_pengguna_tanpa_gambar($nama,$jenkel,$username,$password,$email,$nohp,$level){
		$hsl=$this->db->query("INSERT INTO tbl_pengguna (pengguna_nama,pengguna_jenkel,pengguna_username,pengguna_password,pengguna_email,pengguna_nohp,pengguna_level)
		 VALUES ('$nama','$jenkel','$username',md5('$password'),'$email','$nohp','$level')");
		return $hsl;
	}
